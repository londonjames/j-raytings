...
function ShowFilterBar({ onFilterChange, activeFilter: propActiveFilter, onActiveFiltersChange, onFilterCountChange, onFilterRowsChange }) {
  ...
  const imdbOptions = ['≥9.0', '8.0-8.9', '7.0-7.9', '6.0-6.9', '5.0-5.9', '<5.0']
            ...
            {activeFilters.imdb && (
              <div className="filter-dropdown">
                <button className="filter-type-button active" onClick={(e) => toggleDropdown('imdb', e)}>
                  By IMDb ({selectedFilters.imdb.length})
                </button>
                {openDropdowns.imdb && (
                  <div className="filter-checkbox-dropdown">
                    {imdbOptions.map(imdb => (
                      <label key={imdb} className="filter-checkbox-label">
                        <input
                          type="checkbox"
                          checked={selectedFilters.imdb.includes(imdb)}
                          onChange={() => handleValueToggle('imdb', imdb)}
                          className="filter-checkbox"
                        />
                        <span>{imdb}</span>
                      </label>
